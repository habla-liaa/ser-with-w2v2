...
#Local Encoder
#paiprun configs/main/w2v2-exps.yaml --output_path "s3://lpepino-datasets2/is2021_experiments/w2v2LS960-local/${SEED}" --mods $seeds_mod"&global/wav2vec2_embedding_layer=local_encoder&global/dienen_config=!yaml configs/dienen/mean_mlp.yaml"$ls960_mod
#Contextual Encoder
#paiprun configs/main/w2v2-exps.yaml --output_path "s3://lpepino-datasets2/is2021_experiments/w2v2LS960-contextual/${SEED}" --mods $seeds_mod"&global/wav2vec2_embedding_layer=output&global/dienen_config=!yaml configs/dienen/mean_mlp.yaml"$ls960_mod
#All Layers
#paiprun configs/main/w2v2-exps.yaml --output_path "s3://lpepino-datasets2/is2021_experiments/w2v2LS960-alllayers/${SEED}" --mods $seeds_mod"&global/wav2vec2_embedding_layer=enc_and_transformer"$ls960_mod

#------------------------------------------Ablation--------------------------------------------------
#LSTM
#paiprun experiments/configs/main/w2v2-exps.yaml --output_path "s3://lpepino-datasets2/is2021_experiments/w2v2PT-alllayers-lstm/${SEED}" --mods $seeds_mod"global/wav2vec2_embedding_layer=enc_and_transformer&global/dienen_config=!yaml configs/dienen/feature_learnable_combination_mean_bilstm.yaml&global/batch_size=16"
#-SpeakerNorm
...
errors=1
while (($errors!=0)); do
paiprun experiments/configs/main/w2v2-soapaper-config.yaml --output_path "s3://lpepino-datasets2/is2021_experiments/using_dropout/soapaper-ravdess/${SEED}" --mods "${seed_mod}&global/wav2vec2_embedding_layer=enc_and_transformer&global/normalize=global&global/dataset=ravdess"
errors=$?; done
#IEMOCAP
#errors=1
#while (($errors!=0)); do
#paiprun experiments/configs/main/w2v2-soapaper-config.yaml --output_path "s3://lpepino-datasets2/is2021_experiments/using_dropout/soapaper-ravdess/${SEED}" --mods "${seed_mod}&global/wav2vec2_embedding_layer=enc_and_transformer&global/normalize=global&global/dataset=iemocap_impro"
#errors=$?; done