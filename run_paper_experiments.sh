...
errors=1
while (($errors!=0)); do
paiprun configs/main/os-baseline.yaml --output_path "s3://lpepino-datasets2/is2021_experiments/using_dropout/os-baseline/${SEED}" --mods $seed_mod
errors=$?; done
#Spectrogram-experiment
errors=1
while (($errors!=0)); do
paiprun configs/main/spectrogram-baseline.yaml --output_path "s3://lpepino-datasets2/is2021_experiments/using_dropout/spectrogram-baseline/${SEED}" --mods $seed_mod
errors=$?; done
#-----------------------------------------Wav2Vec2-PT-----------------------------------------------
#Local Encoder
errors=1
while (($errors!=0)); do
paiprun configs/main/w2v2-exps.yaml --output_path "s3://lpepino-datasets2/is2021_experiments/using_dropout/w2v2PT-local/${SEED}" --mods "${seed_mod}&global/wav2vec2_embedding_layer=local_encoder&global/dienen_config=!yaml configs/dienen/mean_mlp.yaml"
errors=$?; done
#Contextual Encoder
errors=1
while (($errors!=0)); do
paiprun configs/main/w2v2-exps.yaml --output_path "s3://lpepino-datasets2/is2021_experiments/using_dropout/w2v2PT-contextual/${SEED}" --mods "${seed_mod}&global/wav2vec2_embedding_layer=output&global/dienen_config=!yaml configs/dienen/mean_mlp.yaml"
errors=$?; done
#All Layers
errors=1
while (($errors!=0)); do
paiprun configs/main/w2v2-exps.yaml --output_path "s3://lpepino-datasets2/is2021_experiments/using_dropout/w2v2PT-alllayers/${SEED}" --mods "${seed_mod}&global/wav2vec2_embedding_layer=enc_and_transformer"
errors=$?; done
#-----------------------------------------Wav2Vec2-LS960---------------------------------------------
...
errors=1
while (($errors!=0)); do
paiprun configs/main/w2v2-os-exps.yaml --output_path "s3://lpepino-datasets2/is2021_experiments/using_dropout/w2v2PT-alllayers-egemaps/${SEED}" --mods "${seed_mod}&global/wav2vec2_embedding_layer=enc_and_transformer&global/normalize=global"
errors=$?; done
